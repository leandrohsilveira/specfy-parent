...
public abstract class AbstractEngine<T extends AbstractRequest> implements Engine {

	private static final Logger LOG = LoggerFactory.getLogger(AbstractEngine.class);

	protected abstract T createRequest(RequestSpec requestSpec, String url, Charset charset) throws IOException;

	@Override
	public Request prepareRequest(RequestSpec requestSpec) throws IOException {
		...
		if (requestSpec.resourceActionSpec.resourceSpec.parameters != null && requestSpec.parameters != null) {
		...
		T request = createRequest(requestSpec, url, charset);
	...
	private void applySslContext(RequestSpec requestSpec, T request) {
	...
	private void applyCookies(Map<String, List<Object>> cookies, T request) {
	...
	private void applyHeaders(Map<String, List<Object>> headers, T request) {
